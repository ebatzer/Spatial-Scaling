...
bind_rows(sig_q0, sig_q2) %>% 
  ...
  labs(title = "N Fertilization Effect on Alpha Diversity") +  
  xlab("Year")+
  ylab("Net Change in Diversity") +
  geom_text(data = data.frame(year = 2018, div = -.05, q = "Effective No. Species"), label = "*", size = 10) +
  geom_text(data = data.frame(year = 2018, div = -.125, q = "Species Richness"), label = "*", size = 10) +
  scale_fill_discrete("Year:", 
                      breaks=c("2017", "2018")) +
  theme_fivethirtyeight()

ggsave("../Figures/AlphaDivChange.jpeg", height = 5, width = 7)
