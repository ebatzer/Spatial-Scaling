...
com.matrix <- com.matrix[order(com.ids$index),]
com.matrix <- com.matrix[,-1]
com.matrix[is.na(com.matrix$index),]

com.matrix <- com.matrix %>% select(-one_of("Rock", "Bare", "Gopher"))
...
for(q.value in c(1:3)){
  ...
  ggsave(paste("./Figures/rawdiff_order", q.value, ".pdf", sep = ""),
         height = 10,
         width = 20)
  ...
  ggsave(paste("/Figures/propdiff_order", q.value, ".pdf", sep = ""),
         height = 10,
         width = 20)
